...
func main() {
	...
	outputFile := flag.String("output", "unicode_truncation_map.json", "Output file name for the JSON map.")
	flag.Parse()
	...
	fmt.Printf("Generating Unicode truncation mappings for character range %d-%d...\n", min, max)
	if *maxTrunc > 0 {
		fmt.Printf("Limiting to a maximum of %d mappings per character.\n", *maxTrunc)
	}

	charMap, err := GenerateTruncationMap(min, max, *maxTrunc)
	if err != nil {
	...
	fmt.Printf("Completed successfully! Found %d Unicode characters that truncate to the target range.\n", totalMappings)
